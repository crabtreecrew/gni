class Reconciler < ActiveRecord::Base
  ...
  def next_batch
    transaction do
      batch = r.reconciler_batches.where(:status => ProgressStatus.working).limit(1).to_sql
      if batch
        batch.status = ProgresStatus.working
        batch.save!
      end
    end
    batch
  ...
  def create_batches
    ...
    offset = 0
    while offset < count
      ReconcilerBatch.create(:reconciler => self, :offset => offset, :status => ProgressStatus.working)
      offset += batch_size
    end
  end
